...
class Nation extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name_th, name_en, short_name, zone, created_date', 'required'),
			array('modified_by', 'numerical', 'integerOnly'=>true),
			array('name_th, name_en', 'length', 'max'=>100),
			array('short_name', 'length', 'max'=>3),
			array('zone, created_by', 'length', 'max'=>10),
			array('image', 'length', 'max'=>300),
			array('status, deleted', 'length', 'max'=>1),
			array('description, modified_date', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name_th, name_en, short_name, zone, image, description, status, deleted, created_date, created_by, modified_date, modified_by', 'safe', 'on'=>'search'),
		);
	}
