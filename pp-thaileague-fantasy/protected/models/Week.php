...
class Week extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name_th, short_name_th, season_id, tournament_id, created_date', 'required'),
			array('modified_by', 'numerical', 'integerOnly'=>true),
			array('name_th, name_en, short_name_th, short_name_en', 'length', 'max'=>100),
			array('fixture_no', 'length', 'max'=>4),
			array('season_id, tournament_id, created_by', 'length', 'max'=>10),
			array('status, deleted', 'length', 'max'=>1),
			array('description, modified_date', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name_th, name_en, short_name_th, short_name_en, fixture_no, season_id, tournament_id, description, status, deleted, created_date, created_by, modified_date, modified_by', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('fixture_no',$this->fixture_no,true);
