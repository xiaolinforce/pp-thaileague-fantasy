...
- Keep lineup management and player discovery in one workspace. Desktop shows
  ...
  past their original position. Each compact workspace remembers its own
  document scroll position so returning to a previously visited tab restores
  the manager's place.
...
- On Mobile and Tablet (below 1280px), tapping an occupied pitch or bench token first
  ...
  Swap retains its existing eligibility constraints. The column chooses the
  available side and stays within the visible viewport. The rest of the page
  is dimmed and inert, with scrolling locked and focus contained. Tapping the
...
- On Desktop (1280px and above), editable occupied-token actions appear in a
  ...
  occupied token still opens player details without exposing disabled hover
  controls. Direct player-detail selection and the existing swap/remove
...
kind of scored lineup. Keep its selected Gameweek in one compact selector and
show one full-width primary-orange total above the pitch; avoid repeating the
selected week, score comparisons, calculation caveats, or provenance in the
presentation layer. The captain and vice-captain badges identify the two
highest-scoring starters in that order when consistent with the optimal score.
The four uncounted bench players are the strongest legal remainder after the
starting eleven, with outfield substitutes ordered by points; their points do
not contribute to the headline total.
